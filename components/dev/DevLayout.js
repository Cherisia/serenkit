...
export default function DevLayout({ toolKey, children, faqs = [] }) {
  const tool = DEV_TOOLS.find(t => t.key === toolKey)
  const related = DEV_TOOLS.filter(t => t.key !== toolKey)

  return (
    <div className="bg-slate-50 pb-20 min-h-screen">
      {/* Hero Banner */}
      <section
        className={`relative bg-gradient-to-br ${tool?.color ?? 'from-slate-600 to-zinc-700'} pt-20 pb-12`}
        aria-label="페이지 소개"
      >
        <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: DEV_HERO_PATTERN }} aria-hidden="true" />
          ...
          <aside className="container 2xl:w-[600px] md:w-[640px] w-[92%] mx-auto mt-10" aria-label="관련 도구">
            <section>
              <h2 className="text-base font-black text-slate-700 mb-4 flex items-center gap-2">
                <span className="w-1 h-5 bg-slate-400 rounded-full inline-block" aria-hidden="true"></span>
                다른 개발자 도구
              </h2>
              <ul className="grid grid-cols-2 md:grid-cols-4 gap-2">
                {related.map(t => (
                  <li key={t.key}>
                    <Link
                      href={t.url}
                      className="flex flex-col items-center gap-1.5 p-3 bg-white border border-slate-200 rounded-xl hover:border-slate-400 hover:shadow-sm transition-all text-center group"
                    >
                      <span className="text-2xl" aria-hidden="true">{t.icon}</span>
                      <span className="text-xs font-bold text-slate-600 group-hover:text-slate-800 leading-tight">{t.shortName}</span>
                    </Link>
                  </li>
                ))}
              </ul>
